...
//
// QueueingFlowSim is the generic single-server flow engine
// (source -> FIFO queue -> server -> sink) driven by the kernel scheduler;
// Mm1Simulator specializes it with exponential(1/lambda) inter-arrivals and
// exponential(1/mu) service times. Every customer's wait time is recorded.
#pragma once
